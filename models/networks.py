...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    if netF == 'global_pool':
        ...
        net = Mlp(init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=16)
    else:
...
class Mlp(nn.Module):
    def __init__(self, use_mlp=True, init_type='normal', init_gain=0.02, gpu_ids=[], nc=16):
    ...
    def create_mlp(self, feats):
        for mlp_id, feat in enumerate(feats):
            input_nc = feat.shape[2]*feat.shape[3]
    ...
    def forward(self, feats):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            feat_reshape = feat.flatten(-2, -1) # B, C, H*W
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        self.opt = opt
        ##### self-attention
        self.use_sa_layers = self.opt.use_sa_layers
        self.sa_blocks = [int(i) for i in self.opt.sa_blocks.split(',')]
        ##### self-attention
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            ##### self-attention
            if self.use_sa_layers and ( i in self.sa_blocks):
               model += [Self_Attn(ngf * mult,'relu')]
               print('self attention was added as %d th layer' %int(len(model)-1))
            ##### self-attention
        ...
        self.model = nn.Sequential(*model)
        print('Total Layer Number : %d' %len(model))
...
class Self_Attn(nn.Module):
    """ Self attention Layer from Self-Attention-GAN [ https://github.com/heykeetae/Self-Attention-GAN/blob/master/sagan_models.py ] """
